...
DELETE="rm -f "
BUCKET="gs://lle-db-app"

gcloud beta container clusters get-credentials cluster-1 --region us-east1 --project cluster-16546 -q

${DELETE} ${backup_dir}/*

kubectl exec $(kubectl get po | grep -e '^dev-postgres' | head -n 1 | awk '{print $1}') -- bash -c "PGPASSWORD=root  pg_dump -U admin -d dev-postgres" > ${backup_dir}/dev1_db_${backup_date}.sql
...
kubectl exec $(kubectl get po | grep -e '^keycloak-postgres' | head -n 1 | awk '{print $1}') -- bash -c "pg_dump -U postgres -d keycloak" > ${backup_dir}/keycloak_db_${backup_date}.sql
